...
public class CandidateDaoImpl implements CandidateDao {
	...
	@Override
	public List<Candidate> getTop2Candidate() throws SQLException {
		List<Candidate> candidateList = new ArrayList<>();
	...
	@Override
	public List<Candidate> getPartyWiseVote() throws SQLException {

		List<Candidate> partyList = new ArrayList<>();
		System.out.println("party exe ");
		try (ResultSet rs = pst4.executeQuery()) {
			while (rs.next())
				partyList.add(new Candidate(rs.getString(1), rs.getInt(2)));
			// partyList.add(rs.getString(1), rs.getInt(2));
